  ...
  Widget build(BuildContext context) {
    return Consumer<InventoryProvider>(
      builder: (context, inventoryProvider, child) {
        // Use watchDishes stream to get real-time updates instead of local _dishes list
        return StreamBuilder<List<dynamic>>(
          stream: inventoryProvider.watchDishes(),
          builder: (context, dishSnapshot) {
            // Also need to watch inventory to check if dishes can be prepared
            return StreamBuilder<List<dynamic>>(
              stream: inventoryProvider.watchIngredients(),
              builder: (context, ingredientSnapshot) {
                if (!dishSnapshot.hasData || !ingredientSnapshot.hasData) {
                  return const Center(child: CircularProgressIndicator());
                }

                final dishes = dishSnapshot.data ?? [];
